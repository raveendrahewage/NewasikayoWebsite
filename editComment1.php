...
<?php 
	if(isset($_POST['submit']))
	{
		 ...
		 if(empty($comPhoto))
		 {
				...
				if($result1)
				{
			 			header( "Location:displayComments1.php?stuNumber=$stuNumber&postId=$postId&mediaName=$mediaName&name=$mediaName&studentNumber=$studentNumber&my=0" );die;
				}
		 }
		 else
		 {
    ...
    <h2>Edit Comment</h2>
		...
		<?php 
			...
			if($usersInfo)
			{
			 ...
			 ?>
			 <form action="editComment1.php?stuNumber=<?php echo $_GET['stuNumber']; ?>&postId=<?php echo $_GET['postId']; ?>&mediaName=<?php echo $_GET['mediaName']; ?>&name=<?php echo $_GET['name']; ?>&commentId=<?php echo $_GET['commentId']; ?>&my=0" method="post" class="editCom" enctype="multipart/form-data">
